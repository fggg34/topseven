    ...
    <form method="POST" action="{{ route('password.store') }}" class="space-y-5">
        ...
        <div>
            ...
            <input id="email" type="email" name="email" value="{{ old('email', $request->email) }}" required autofocus autocomplete="username"
                class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3.5 text-[15px] text-brand-ink focus:outline-none focus:ring-2 focus:ring-[#3f4b9a] focus:border-transparent transition" />
