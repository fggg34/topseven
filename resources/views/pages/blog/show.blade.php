...
<article class="max-w-[900px] mx-auto px-4 sm:px-6 lg:px-8 py-14">
    ...
    @endif
    <div class="blog-content prose prose-lg max-w-none prose-headings:font-serif prose-headings:text-brand-heading prose-p:text-[#4a4a4a] prose-p:leading-[1.8] prose-a:text-brand-ink prose-a:underline prose-a:underline-offset-4">
        {!! $post->content_html !!}
    </div>
        ...
        <div class="flex flex-wrap flex-col items-left gap-4 text-sm">
            @if($post->category)
                <div class="flex items-center gap-3">
                    <span class="text-[11px] font-semibold uppercase tracking-wider text-brand-ink/50">Category</span>
                    <a href="{{ route('blog.index', ['category' => $post->category->slug]) }}" class="inline-flex items-center px-4 py-1.5 bg-[#f8f6f2] text-brand-ink text-sm font-semibold hover:bg-[#eee9df] transition-colors">{{ $post->category->name }}</a>
                </div>
            @endif
            @if($post->tags->isNotEmpty())
                <div class="flex items-center gap-3 flex-wrap">
                    <span class="text-[11px] font-semibold uppercase tracking-wider text-brand-ink/50">Tags</span>
                    <div class="flex flex-wrap gap-2">
                        @foreach($post->tags as $tag)
                            <a href="{{ route('blog.index', ['tag' => $tag->slug]) }}" class="inline-flex items-center px-4 py-1.5 bg-[#f8f6f2] text-brand-ink text-sm font-medium hover:bg-[#eee9df] transition-colors">{{ $tag->name }}</a>
                        @endforeach
                    </div>
                </div>
            @endif
...
<section class="border-t border-[#e6e1d8] bg-[#f8f6f2]">
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <h2 class="text-3xl font-serif text-brand-heading mb-8">Related articles</h2>
            ...
            @foreach($related as $p)
                ...
                <article class="group">
                    <a href="{{ route('blog.show', $p->slug) }}" class="block">
                        ...
                        <div class="pt-5">
                            <p class="text-[12px] text-brand-ink/50 uppercase tracking-wider mb-2">{{ ($p->published_at ?? $p->created_at)?->format('M d, Y') }}</p>
                            <h3 class="text-xl font-serif text-brand-heading line-clamp-2 leading-snug group-hover:underline decoration-1 underline-offset-4">{{ $p->title }}</h3>
                        </div>
                    </a>
                </article>
            @endforeach
