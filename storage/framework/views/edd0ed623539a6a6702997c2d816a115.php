...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['tour', 'queryParams' => [], 'wishlisted' => false, 'slider' => false, 'variant' => 'default']));
...
foreach (array_filter((['tour', 'queryParams' => [], 'wishlisted' => false, 'slider' => false, 'variant' => 'default']), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($variant === 'flash'): ?>
<?php
...
?>
<article <?php echo e($attributes->merge(['class' => 'h-full min-w-0 w-full'])); ?>>
    <a href="<?php echo e($tourUrl); ?>" class="home-flash-sale-card group relative block w-full max-w-full aspect-[3/4] min-h-[420px] sm:min-h-[480px] max-h-[560px] rounded-[28px] overflow-hidden shadow-md ring-1 ring-black/10 h-full box-border">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($wishlisted): ?>
                <form method="POST" action="<?php echo e(route('wishlist.destroy', $tour)); ?>" class="absolute top-4 right-4 z-20" onclick="event.preventDefault(); event.stopPropagation(); this.submit();">
                    <?php echo csrf_field(); ?>
                    <?php echo method_field('DELETE'); ?>
                    <button type="submit" class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center text-rose-400 hover:bg-white/30 transition-colors" aria-label="<?php echo e(__('Remove from wishlist')); ?>">
            ...
            <?php else: ?>
                <form method="POST" action="<?php echo e(route('wishlist.store', $tour)); ?>" class="absolute top-4 right-4 z-20" onclick="event.preventDefault(); event.stopPropagation(); this.submit();">
                    <?php echo csrf_field(); ?>
                    <button type="submit" class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center text-white hover:text-rose-400 hover:bg-white/30 transition-colors" aria-label="<?php echo e(__('Add to wishlist')); ?>">
        ...
        <div class="absolute inset-x-0 bottom-0 z-10 p-5 sm:p-6 pt-24 flex flex-col justify-end text-left min-w-0">
            ...
            <div class="mt-4 flex flex-wrap items-baseline gap-x-2 gap-y-1">
                <span class="text-[15px] sm:text-base font-bold text-white tabular-nums tracking-tight">
                    <?php echo e(__('From')); ?> <?php echo e($currency); ?><?php echo e(number_format($sale, $decimals)); ?>
...
<article <?php echo e($attributes->merge(['class' => 'group bg-white rounded-xl overflow-hidden border border-gray-200 transition-shadow min-w-0 w-full ' . ($slider ? 'flex-shrink-0 hover:shadow-md' : 'hover:shadow-lg')])); ?>

    <?php if($slider): ?> data-slider-card <?php endif; ?>>
    <a href="<?php echo e($tourUrl); ?>" class="flex flex-col h-full w-full min-w-0">
        ...
        <div class="relative overflow-hidden flex-shrink-0 aspect-[4/3] rounded-xl px-2 mt-2" style="max-width: 100% !important;">
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($tour->season): ?>
                <div class="absolute bottom-3 left-3 flex flex-wrap gap-1.5">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($tour->is_featured): ?>
                        <span class="inline-flex items-center px-2.5 py-1 text-xs font-bold bg-brand-btn text-white rounded-md backdrop-blur-sm"><?php echo e(__('Best Seller')); ?></span>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    <?php
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($hasDiscount && $discountPercent >= 5): ?>
                <div class="absolute top-3 left-3">
                    <span class="inline-flex items-center px-2 py-0.5 text-[11px] font-bold bg-red-500 text-white rounded"><?php echo e($discountPercent); ?>% <?php echo e(__('Off')); ?></span>
                </div>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($wishlisted): ?>
                    <form method="POST" action="<?php echo e(route('wishlist.destroy', $tour)); ?>" class="absolute top-3 right-3 z-10" onclick="event.preventDefault(); event.stopPropagation(); this.submit();">
                        <?php echo csrf_field(); ?>
                        <?php echo method_field('DELETE'); ?>
                        <button type="submit" class="w-9 h-9 rounded-xl bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center text-rose-500 hover:bg-white/30 transition-colors" aria-label="<?php echo e(__('Remove from wishlist')); ?>">
                            <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><path d="M6.25 3.333a4.583 4.583 0 0 0-4.583 4.584c0 4.583 5.416 8.75 8.333 9.716 2.917-.966 8.333-5.133 8.333-9.716A4.583 4.583 0 0 0 10 5.28a4.578 4.578 0 0 0-3.75-1.948Z" fill="currentColor" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                        </button>
                    </form>
                <?php else: ?>
                    <form method="POST" action="<?php echo e(route('wishlist.store', $tour)); ?>" class="absolute top-3 right-3 z-10" onclick="event.preventDefault(); event.stopPropagation(); this.submit();">
                        <?php echo csrf_field(); ?>
                        <button type="submit" class="w-9 h-9 rounded-xl bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center text-white hover:text-rose-500 hover:bg-white/30 transition-colors" aria-label="<?php echo e(__('Add to wishlist')); ?>">
                            <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><path d="M6.25 3.333a4.583 4.583 0 0 0-4.583 4.584c0 4.583 5.416 8.75 8.333 9.716 2.917-.966 8.333-5.133 8.333-9.716A4.583 4.583 0 0 0 10 5.28a4.578 4.578 0 0 0-3.75-1.948Z" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                        </button>
                    </form>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
...
<?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
<?php /**PATH /Users/kevinhitaj/Desktop/Projects/TOP 7 TRAVEL/resources/views/components/tour-card.blade.php ENDPATH**/ ?>